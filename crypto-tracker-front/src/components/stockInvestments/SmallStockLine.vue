<template>
  <v-card>
    <v-card-title class="lighten-2" style="justify-content: space-between">
      <div class="text-h5">{{ stockName }} <span :style="`color: ${ percDiff > 0 ? 'green' : 'red'}`">{{ (percDiff).toFixed(3) }}%</span></div>
...
export default {
  ...
  methods: {
    populateChart(numOfDays) {
      let items = this.$store.state.stockPrices.filter(d => d.prices.some(c => c.symbol === this.stockName)).slice(-Math.abs(numOfDays))
      this.chartData.labels = items.map(h => h.updatedAt.slice(2, 10))
      this.chartData.datasets[0].data = items.map(h => h.prices.filter(c => c.symbol === this.stockName).map(v => v.price).reduce((prev, next) => prev + next, 0))
    }
  },
  data() {
    return {
